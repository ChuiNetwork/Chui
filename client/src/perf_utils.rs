use {
    log::*,
    chui_sdk::{client::Client, commitment_config::CommitmentConfig, timing::duration_as_s},
    std::{
        sync::{
    ...
    T: Client,
{
    ...
    loop {
        ...
        now = Instant::now();
        let mut txs = match client.get_transaction_count_with_commitment(CommitmentConfig::processed()) {
            Err(e) => {
