...
use chui_account_program::{ChuiAccountType, ACCOUNT_LEN as CHUI_ACCOUNT_SIZE};
use chui_relying_party_program::RelyingPartyData;
...
impl JsonRpcRequestProcessor {
    ...
    fn get_chui_accounts_by_storage_key(
        &self,
        bank: &Arc<Bank>,
        storage_key: Pubkey,
        ...
        let is_target_chui_account = |account: &AccountSharedData| -> bool {
            *account.owner() == chui_account_program::id()
                && account.data().len() == CHUI_ACCOUNT_SIZE
                && matches!(
                    ChuiAccountType::try_from(account.data()),
                    Ok(ChuiAccountType::Account(_account_info))
                )
        };

        self.config
            .account_indexes
            .contains(&AccountIndex::ChuiAccountStorage)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::ChuiAccountStorage(storage_key),
                    is_target_chui_account,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
            })
    ...
    fn get_chui_accounts_by_owner_key(
        &self,
        bank: &Arc<Bank>,
        owner_key: Pubkey,
    ) -> Option<ScanResult<Vec<(Pubkey, AccountSharedData)>>> {
        let is_target_chui_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == chui_account_program::id()
                && match ChuiAccountType::try_from(account.data()) {
                    Ok(ChuiAccountType::Account(account)) => account.owners.contains(&owner_key),
                    _ => false,
                }
        };
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::ChuiAccountOwner)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::ChuiAccountOwner(owner_key),
                    is_target_chui_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
            })
    ...
    fn get_chui_relying_party_by_owner_key(
        &self,
        bank: &Arc<Bank>,
        owner_key: Pubkey,
    ) -> Option<ScanResult<Vec<(Pubkey, AccountSharedData)>>> {
        let is_target_chui_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == chui_relying_party_program::id()
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::ChuiRelyingOwner)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::ChuiRelyingOwner(owner_key),
                    is_target_chui_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
    ...
    fn get_chui_accounts_storages_by_operational_key(
        &self,
        bank: &Arc<Bank>,
        ...
        let is_target_chui_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == chui_account_program::id()
                && match ChuiAccountType::try_from(account.data()) {
                    Ok(ChuiAccountType::Storage(account_storage)) => account_storage
                        .operationals
                        .iter()
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::ChuiAccountOperational)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::ChuiAccountOperational(operational_key),
                    is_target_chui_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
...
pub mod rpc_minimal {
    ...
    pub trait Minimal {
        ...
        // TODO: Refactor `chui-validator wait-for-restart-window` to not require this method, so
...
pub mod rpc_accounts {
    ...
    pub trait AccountsData {
        ...
        #[rpc(meta, name = "getChuiAccountsByOperationalKey")]
        fn get_chui_accounts_by_operational_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ) -> Result<RpcResponse<Vec<String>>>;

        #[rpc(meta, name = "getChuiAccountsByOwnerKey")]
        fn get_chui_accounts_by_owner_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ...
        #[rpc(meta, name = "getChuiRelyingPartiesByOwnerKey")]
        fn get_chui_relying_parties_by_owner_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ) -> Result<RpcResponse<Vec<String>>>;
    ...
    impl AccountsData for AccountsDataImpl {
        ...
        fn get_chui_accounts_by_owner_key(
            ...
            debug!(
                "get_chui_accounts_by_owner_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::ChuiAccountOwner);

            let owner_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let accounts = meta.get_chui_accounts_by_owner_key(&bank, owner_key).transpose().map_err(|e|RpcCustomError::ScanError {
                message: e.to_string(),
            })?;
            debug!(
                "get_chui_accounts_by_owner_key chui accounts {:?}",
                accounts
            );
        ...
        fn get_chui_relying_parties_by_owner_key(
            ...
            debug!(
                "get_chui_relying_parties_by_owner_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::ChuiRelyingOwner);

            let owner_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let accounts = meta.get_chui_relying_party_by_owner_key(&bank, owner_key).transpose().map_err(|e|RpcCustomError::ScanError {
                message: e.to_string(),
            })?;
            debug!(
                "get_chui_relying_parties_by_owner_key chui accounts storages {:?}",
                accounts
            );
        ...
        fn get_chui_accounts_by_operational_key(
            ...
            debug!(
                "get_chui_accounts_by_operational_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::ChuiAccountOperational);
            check_index!(&meta.config, AccountIndex::ChuiAccountStorage);

            let operational_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let storages =
                meta.get_chui_accounts_storages_by_operational_key(&bank, operational_key).transpose().map_err(|e|RpcCustomError::ScanError {
                    message: e.to_string(),
                })?;
            debug!(
                "get_chui_accounts_by_operational_key chui accounts storages {:?}",
                storages
            );

            let accounts: std::result::Result<Vec<_>,_> = storages
                .expect("chui account operational index to be activated")
                .into_iter()
                .map(|(storage, _)| {
                    meta.get_chui_accounts_by_storage_key(&bank, storage)
                        .expect("chui account storage index to be activated")
                })
                .collect();
            ...
            debug!(
                "get_chui_accounts_by_operational_key chui accounts {:?}",
                accounts
            );
