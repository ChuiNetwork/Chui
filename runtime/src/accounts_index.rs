...
use chui_account_program::{VAccountStorage, ChuiAccountType};
...
pub enum IndexKey {
    ...
    ChuiAccountStorage(Pubkey),
    ChuiAccountOwner(Pubkey),
    ChuiAccountOperational(Pubkey),
    ChuiRelyingOwner(Pubkey),
}
...
impl AccountIndex {
    pub fn name(&self) -> &str {
        match self {
            ...
            AccountIndex::ChuiAccountStorage => "Chui Account Storage",
            AccountIndex::ChuiAccountOwner => "Chui Account Owner",
            AccountIndex::ChuiAccountOperational => "Chui Account Operational",
            AccountIndex::ChuiRelyingOwner => "Chui Relying Owner",
        }
    }
}
...
pub struct AccountsIndex<T: IndexValue> {
    ...
    // Chui Indices
    chui_account_storage_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    chui_account_owner_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    chui_account_operational_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    chui_relying_party_owner_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
...
impl<T: IndexValue> AccountsIndex<T> {
    ...
    pub fn new(config: Option<AccountsIndexConfig>) -> Self {
        ...
        Self {
            ...
            //Chui indexes
            chui_account_storage_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("chui_account_storage_index"),
            chui_account_owner_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("chui_account_owner_index"),
            chui_account_operational_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("chui_account_operational_index"),
            chui_relying_party_owner_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("chui_relying_party_owner_index"),
        }
    }
        ...
        R: RangeBounds<Pubkey> + std::fmt::Debug,
    {
        ...
        */
        match scan_type {
            ...
            ScanTypes::Indexed(IndexKey::ChuiAccountStorage(va_storage_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.chui_account_storage_index,
                    &va_storage_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::ChuiAccountOwner(va_owner_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.chui_account_owner_index,
                    &va_owner_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::ChuiAccountOperational(va_operational_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.chui_account_operational_index,
                    &va_operational_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::ChuiRelyingOwner(va_owner_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.chui_relying_party_owner_index,
                    &va_owner_key,
                    Some(max_root),
                    config,
                ),
    ...
    pub(crate) fn update_secondary_indexes(
        ...
        // Chui account
        if *account_owner == chui_account_program::id() {
            match ChuiAccountType::try_from(account_data) {
                Ok(ChuiAccountType::Account(account_info)) => {
                    if account_indexes.contains(&AccountIndex::ChuiAccountStorage) {
                        let storage = account_info.find_storage_key(pubkey);
                        self.chui_account_storage_index.insert(&storage, pubkey);
                    }

                    if account_indexes.contains(&AccountIndex::ChuiAccountOwner) {
                        for owner in account_info.owners {
                            if owner != Pubkey::default() {
                                self.chui_account_owner_index.insert(&owner, pubkey);
                            }
                        }
                    }
                }
                Ok(ChuiAccountType::Storage(VAccountStorage { operationals })) => {
                    if account_indexes.contains(&AccountIndex::ChuiAccountOperational) {
                ...
                Err(err) => log::warn!("Unable to parse Chui Account: {:?}", err),
            }
        }

        // Chui relying party data
        if *account_owner == chui_relying_party_program::id() {
            match RelyingPartyData::try_from(account_data) {
                Ok(acc) => {
                    if account_indexes.contains(&AccountIndex::ChuiRelyingOwner) {
                        self.chui_relying_party_owner_index
