...
pub use chui_perf::report_target_features;
use {
    crate::{
    ...
    rand::{thread_rng, Rng},
    chui_entry::poh::compute_hash_time_ns,
    chui_geyser_plugin_manager::geyser_plugin_service::GeyserPluginService,
    chui_gossip::{
        cluster_info::{
    ...
    chui_send_transaction_service::send_transaction_service,
    chui_streamer::socket::SocketAddrSpace,
    chui_vote_program::vote_state::VoteState,
    std::{
...
pub struct Validator {
    ...
    tpu: Tpu,
    tvu: Tvu,
    ip_echo_server: Option<chui_net_utils::IpEchoServer>,
...
impl Validator {
    ...
    pub fn new(
        ...
        if chui_perf::perf_libs::api().is_some() {
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::StartingServices;
        ...
        let ip_echo_server = match node.sockets.ip_echo {
            None => None,
            Some(tcp_listener) => Some(chui_net_utils::ip_echo_server(
                tcp_listener,
        ...
        datapoint_info!(
            ...
            ("version", chui_version::version!(), String)
        );
...
fn new_banks_from_ledger(
    ...
    *start_progress.write().unwrap() = ValidatorStartProgress::LoadingLedger;
    ...
    let evm_genesis_path = ledger_path.join(chui_sdk::genesis_config::EVM_GENESIS);
    ...
    if let Some(warp_slot) = config.warp_slot {
        ...
        bank_forks.set_root(
            warp_slot,
            &chui_runtime::accounts_background_service::AbsRequestSender::default(),
            Some(warp_slot),
        );
...
mod tests {
    use {
        ...
        chui_ledger::{create_new_tmp_ledger, genesis_utils::create_genesis_config_with_leader},
        chui_runtime::snapshot_utils::EVM_STATE_DIR,
        chui_sdk::{genesis_config::create_genesis_config, poh_config::PohConfig},
    ...
    fn test_backup_and_clear_blockstore() {
        ...
        use {
            chui_entry::entry,
            chui_ledger::{blockstore, get_tmp_ledger_path},
        };
    ...
    fn test_poh_speed() {
        ...
        let poh_config = PohConfig {
            target_tick_duration: Duration::from_millis(chui_sdk::clock::MS_PER_TICK),
            // make PoH rate really fast to cause the panic condition
            hashes_per_tick: Some(100 * chui_sdk::clock::DEFAULT_HASHES_PER_TICK),
            ..PohConfig::default()
        };
