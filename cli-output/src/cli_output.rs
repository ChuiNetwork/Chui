use {
    ...
    chui_account_decoder::parse_token::UiTokenAccount,
    chui_clap_utils::keypair::SignOnly,
    chui_client::rpc_response::{
...
impl fmt::Display for CliValidators {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        fn write_vote_account(
            ...
            let style = if enable_majority_filtering && validator.activated_stake <= chui_vote_program::MIN_STAKERS_TO_BE_MAJORITY {
                style(output).dim()
            } else {
                style(output)
            };
        ...
        for (i, validator) in sorted_validators.iter().enumerate() {
            ...
            write_vote_account(
                ...
                highest_last_vote,
                highest_root,
                self.majority_count >= chui_vote_program::NUM_MAJOR_STAKERS_FOR_FILTERING as u64
            )?;
        }
            ...
            match &keyed_reward.reward {
                Some(reward) => {
                    writeln!(
                        ...
                        lamports_to_cui(reward.amount),
                        lamports_to_cui(reward.post_balance),
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for account in &self.accounts {
            writeln!(
                ...
                "{:<44}  {}",
                account.address,
                &format!("{} CUI", lamports_to_cui(account.lamports))
            )?;
        }
        Ok(())
    }
...
impl fmt::Display for CliSupply {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln_name_value(f, "Total:", &format!("{} CUI", lamports_to_cui(self.total)))?;
        writeln_name_value(
            f,
            "Circulating:",
            &format!("{} CUI", lamports_to_cui(self.circulating)),
        )?;
        writeln_name_value(
            f,
            "Non-Circulating:",
            &format!("{} CUI", lamports_to_cui(self.non_circulating)),
        )?;
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if !self.encoded_confirmed_block.rewards.is_empty() {
            ...
            for reward in rewards {
                ...
                writeln!(
                    ...
                    format!(
                        "{}◎{:<14.9}",
                        sign,
                        lamports_to_cui(reward.lamports.unsigned_abs())
                    ),
            ...
            writeln!(
                ...
                "Total Rewards: {}◎{:<12.9}",
                sign,
                lamports_to_cui(total_rewards.unsigned_abs())
            )?;
        }
