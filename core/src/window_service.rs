...
use {
    ...
    chui_metrics::{inc_new_counter_debug, inc_new_counter_error},
    chui_perf::packet::{Packet, PacketBatch},
    chui_rayon_threadlimit::get_thread_count,
    chui_runtime::{bank::Bank, bank_forks::BankForks},
    chui_sdk::{clock::Slot, packet::PACKET_DATA_SIZE, pubkey::Pubkey},
    std::{
        cmp::Reverse,
...
impl WindowService {
    ...
    fn start_check_duplicate_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("chui-check-duplicate-error", 1, 1);
        };
        Builder::new()
            .name("chui-check-duplicate".to_string())
            .spawn(move || loop {
    ...
    fn start_window_insert_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("chui-window-insert-error", 1, 1);
        };

        Builder::new()
            .name("chui-window-insert".to_string())
            .spawn(move || {
            ...
            + std::marker::Sync,
    {
        ...
        Builder::new()
            ...
            .spawn(move || {
                ...
                let handle_error = || {
                    inc_new_counter_error!("chui-window-error", 1, 1);
                };
...
mod test {
    use {
        super::*,
        chui_entry::entry::{create_ticks, Entry},
        chui_gossip::contact_info::ContactInfo,
        chui_ledger::{
    ...
    fn test_should_retransmit_and_persist() {
        let me_id = chui_sdk::pubkey::new_rand();
