...
airdrops_enabled=1
node_sol=500 # 500 SOL: number of CUI to airdrop the node for transaction fees and vote account rent exemption (ignored if airdrops_enabled=0)
label=
identity=
vote_account=
no_restart=0
...
OPTIONS:
  ...
  --node-cui CUI            - Number of CUI this node has been funded from the genesis config (default: $node_sol)
...
if [[ -n $SOLANA_CUDA ]]; then
  program=$chui_validator_cuda
else
  program=$chui_validator
fi
...
wallet() {
  (
    set -x
    $chui_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
...
rpc_url=$($chui_gossip $maybe_allow_private_addr rpc-url --timeout 180 --entrypoint "$gossip_entrypoint")

[[ -r "$identity" ]] || $chui_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $chui_keygen new --no-passphrase -so "$vote_account"
[[ -r "$authorized_withdrawer" ]] || $chui_keygen new --no-passphrase -so "$authorized_withdrawer"
