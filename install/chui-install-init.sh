...
{ # this ensures the entire script is downloaded #
    ...
    if [ -z "$SOLANA_DOWNLOAD_ROOT" ]; then
        SOLANA_DOWNLOAD_ROOT="https://github.com/ChuiNetwork/Chui/releases/download/"
    fi
    GH_LATEST_RELEASE="https://api.github.com/repos/chui/Chui/releases/latest"
...
USAGE:
    chui-install-init [FLAGS] [OPTIONS] --data_dir <PATH> --pubkey <PUBKEY>
...
EOF
    ...
    main() {
        ...
        temp_dir="$(mktemp -d 2>/dev/null || ensure mktemp -d -t chui-install-init)"
        ...
        download_url="$SOLANA_DOWNLOAD_ROOT/$release/chui-install-init-$TARGET"
        solana_install_init="$temp_dir/chui-install-init"
        ...
        if [ ! -x "$solana_install_init" ]; then
            ...
            printf '%s\n' "Please copy the file to a location where you can execute binaries and run ./chui-install-init." 1>&2
            exit 1
        fi
        ...
        if [ -z "$1" ]; then
            ...
            elif [ "$1" = "LATEST_CHUI_RELEASE" ]; then
            ignore "$solana_install_init" $SOLANA_INSTALL_INIT_ARGS "$(echo $release | cut -c2-)"
        else
