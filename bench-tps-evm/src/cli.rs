...
use chui_faucet::faucet::FAUCET_PORT;
...
const NUM_LAMPORTS_PER_ACCOUNT_DEFAULT: u64 = chui_sdk::native_token::LAMPORTS_PER_CUI;
...
pub fn extract_args(matches: &ArgMatches) -> Config {
    ...
    if let Some(addr) = matches.value_of("entrypoint") {
        args.entrypoint_addr = chui_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
    ...
    if let Some(addr) = matches.value_of("faucet") {
        args.faucet_addr = chui_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
