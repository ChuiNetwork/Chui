use {
    ...
    chui_ledger::{blockstore::Blockstore, get_tmp_ledger_path},
    chui_rpc::{
...
fn test_rpc_client() {
    ...
    assert_eq!(
        client.get_version().unwrap().chui_core,
        chui_version::semver!()
    );
    ...
    let tx = system_transaction::transfer(&alice, &bob_pubkey, cui_to_lamports(20.0), blockhash);
    ...
    assert_eq!(
        ...
        cui_to_lamports(20.0)
    );
    assert_eq!(
        client.get_balance(&alice.pubkey()).unwrap(),
        original_alice_balance - cui_to_lamports(20.0)
    );
}
