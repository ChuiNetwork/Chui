...
pub fn lamports_of_cui(matches: &ArgMatches<'_>, name: &str) -> Option<u64> {
    value_of(matches, name).map(cui_to_lamports)
}
...
mod tests {
    ...
    #[test]
    fn test_lamports_of_cui() {
        let matches = app()
            .clone()
            .get_matches_from(vec!["test", "--single", "50"]);
        assert_eq!(lamports_of_cui(&matches, "single"), Some(50_000_000_000));
        assert_eq!(lamports_of_cui(&matches, "multiple"), None);
        let matches = app()
            .clone()
            .get_matches_from(vec!["test", "--single", "1.5"]);
        assert_eq!(lamports_of_cui(&matches, "single"), Some(1_500_000_000));
        assert_eq!(lamports_of_cui(&matches, "multiple"), None);
        let matches = app()
            .clone()
            .get_matches_from(vec!["test", "--single", "0.03"]);
        assert_eq!(lamports_of_cui(&matches, "single"), Some(30_000_000));
    }
}
