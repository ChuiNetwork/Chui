...
pub fn normalize_to_url_if_moniker<T: AsRef<str>>(url_or_moniker: T) -> String {
    match url_or_moniker.as_ref() {
        "m" | "mainnet" => "https://rpc-main-1.chui.network",
        "t" | "testnet" => "https://rpc-test-1.chui.network",
        "d" | "devnet" => "https://rpc-dev-1.chui.network",
    ...
    T: AsRef<str> + Display,
{
    ...
    if chui_perf::thread::is_renice_allowed(adjustment) {
        Ok(())
    } else {
        Err(String::from(
