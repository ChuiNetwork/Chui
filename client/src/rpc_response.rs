use {
    crate::client_error,
    chui_account_decoder::{parse_token::UiTokenAmount, UiAccount},
    chui_sdk::{
...
pub struct RpcVersionInfo {
    /// The current version ofchui-core
    pub chui_core: String,
    /// first 4 bytes of the FeatureSet identifier
    pub feature_set: Option<u32>,
}
...
impl fmt::Debug for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.chui_core)
    }
}

impl fmt::Display for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if let Some(version) = self.chui_core.split_whitespace().next() {
