...
impl BroadcastRun for FailEntryVerificationBroadcastRun {
    fn run(
        ...
        // 2) If we're past SLOT_TO_RENZTVE, insert the correct shreds so validators can repair
        // and make progress
        if bank.slot() > SLOT_TO_RENZTVE && !self.good_shreds.is_empty() {
