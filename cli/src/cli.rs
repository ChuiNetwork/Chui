use {
    ...
    chui_clap_utils::{self, input_parsers::*, input_validators::*, keypair::*},
    chui_cli_output::{
    ...
    chui_remote_wallet::remote_wallet::RemoteWalletManager,
    chui_sdk::{
        clock::{Epoch, Slot},
    ...
    chui_vote_program::vote_state::VoteAuthorize,
...
impl CliConfig<'_> {
    fn default_keypair_path() -> String {
        chui_cli_config::Config::default().keypair_path
    }

    fn default_json_rpc_url() -> String {
        chui_cli_config::Config::default().json_rpc_url
    }

    fn default_websocket_url() -> String {
        chui_cli_config::Config::default().websocket_url
    }
    ...
    pub fn compute_websocket_url_setting(
        ...
        Self::first_nonempty_setting(vec![
            ...
            (
                ...
                chui_cli_config::Config::compute_websocket_url(&normalize_to_url_if_moniker(
                    json_rpc_cmd_url,
                )),
            ),
            (
...
pub fn process_command(config: &CliConfig) -> ProcessResult {
    ...
    match &config.command {
        ...
        // Return software version ofchui-cli and cluster entrypoint node
        CliCommand::Catchup {
            node_pubkey,
            node_json_rpc_url,
            follow,
...
mod tests {
    use {
        ...
        chui_transaction_status::TransactionConfirmationStatus,
        std::path::PathBuf,
    };
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(
            signer_info.index_of(Some(chui_sdk::pubkey::new_rand())),
            None
        );
    ...
    fn test_cli_parse_command() {
        ...
        let from_pubkey = Some(chui_sdk::pubkey::new_rand());
        let from_str = from_pubkey.unwrap().to_string();
        for (name, program_id) in &[
            ("STAKE", stake::program::id()),
            ("VOTE", chui_vote_program::id()),
    ...
    fn test_cli_process_command() {
        ...
        assert_eq!(process_command(&config).unwrap(), "0.0000004 CUI");
        ...
        let new_authorized_pubkey = chui_sdk::pubkey::new_rand();
        ...
        let bob_keypair = Keypair::new();
        let bob_pubkey = bob_keypair.pubkey();
        let custodian = chui_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = chui_sdk::pubkey::new_rand();
        let to_pubkey = chui_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = chui_sdk::pubkey::new_rand();
        let source_stake_account_pubkey = chui_sdk::pubkey::new_rand();
        ...
        // CreateAddressWithSeed
        let from_pubkey = chui_sdk::pubkey::new_rand();
        config.signers = vec![];
