...
use chui_sdk::{
    ...
    message::Message,
    native_token::{lamports_to_sol, LAMPORTS_PER_CUI},
...
use chui_evm_loader_program::{instructions::FeePayerType, scope::evm::gweis_to_lamports};
...
fn get_evm_balance(rpc_client: &RpcClient, address: evm::H160) -> anyhow::Result<()> {
    ...
    let xzo = lamports_to_cui(lamports);
...
fn transfer(
    ...
    let ixs =
        chui_evm_loader_program::transfer_native_to_evm_ixs(from.pubkey(), amount, evm_address);
...
fn find_block_header(
    ...
    for slot in native_slot - range..native_slot + range {
        ...
        let hash = chui_sdk::hash::Hash::from_str(&native_block.blockhash)?;
...
fn send_raw_tx<P: AsRef<Path>>(
    ...
    let tx: evm::Transaction = chui_sdk::program_utils::limited_deserialize(&bytes)?;
    debug!("loaded tx: {:?}", tx);

    let ix = chui_evm_loader_program::send_raw_tx(signer.pubkey(), tx, None, FeePayerType::Evm);
...
fn call_dummy(
    ...
    let evm_tx: evm::Transaction = chui_sdk::program_utils::limited_deserialize(&create_tx)?;
