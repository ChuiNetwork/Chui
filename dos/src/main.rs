...
use {
    ...
    chui_core::serve_repair::RepairProtocol,
    chui_gossip::{contact_info::ContactInfo, gossip_service::discover},
...
fn get_repair_contact(nodes: &[ContactInfo]) -> ContactInfo {
    ...
    contact.id = chui_sdk::pubkey::new_rand();
    contact
}
...
fn run_dos(
    ...
    match data_type.as_str() {
        ...
        "transaction" => {
            let tx = chui_perf::test_tx::test_tx();
            info!("{:?}", tx);
...
fn main() {
    ...
    if let Some(addr) = matches.value_of("entrypoint") {
        entrypoint_addr = chui_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
...
#[cfg(test)]
pub mod test {
    use {super::*, chui_sdk::timing::timestamp};
