...
use derivative::*;
use chui_evm_loader_program::instructions::FeePayerType;
use chui_evm_loader_program::scope::*;
use chui_sdk::{
    clock::MS_PER_TICK,
...
pub struct EvmBridge {
    evm_chain_id: u64,
    key: chui_sdk::signature::Keypair,
...
impl EvmBridge {
    fn new(
        ...
        let key = chui_sdk::signature::read_keypair_file(&keypath).unwrap();
    ...
    /// Wrap evm tx into chui, optionally add meta keys, to chuisignature.
    async fn send_tx(
        &self,
        tx: evm::Transaction,
    ...
    fn make_send_tx_instructions(
        ...
        let mut ix = if self.borsh_encoding {
            ...
            chui_evm_loader_program::send_raw_tx(
                self.key.pubkey(),
                tx.clone(),
            ...
            chui_evm_loader_program::send_raw_tx_old(
                self.key.pubkey(),
                tx.clone(),
                Some(self.key.pubkey()),
            )
        ...
        if native_fee_used {
            vec![
                system_instruction::assign(&self.key.pubkey(), &chui_sdk::evm_loader::ID),
                ix,
                chui_evm_loader_program::free_ownership(self.key.pubkey()),
            ]
        } else {
            vec![ix]
        }
    ...
    fn make_send_big_tx_instructions(
        ...
        let ix = if self.borsh_encoding {
            ...
            chui_evm_loader_program::big_tx_execute(storage_pubkey, Some(&payer_pubkey), fee_type)
        } else {
            chui_evm_loader_program::big_tx_execute_old(storage_pubkey, Some(&payer_pubkey))
        };
        if native_fee_used {
            vec![
                system_instruction::assign(&self.key.pubkey(), &chui_sdk::evm_loader::ID),
...
impl BridgeERPC for BridgeErpcImpl {
    ...
    fn sign(&self, meta: Self::Metadata, address: Hex<Address>, data: Bytes) -> EvmResult<Bytes> {
        ...
        let hash_to_sign = chui_sdk::keccak::hash(&message_data);
    ...
    fn send_transaction(
        ...
        let future = async move {
            ...
            let meta_keys = meta_keys
                .into_iter()
                .flatten()
                .map(|s| chui_sdk::pubkey::Pubkey::from_str(&s))
...
pub(crate) fn from_client_error(client_error: ClientError) -> evm_rpc::Error {
    ...
    match client_error_kind {
        ClientErrorKind::RpcError(chui_client::rpc_request::RpcError::RpcResponseError {
            code,
            message,
            data,
            original_err,
...
impl Args {
    fn min_gas_price_or_default(&self) -> U256 {
        ...
        fn min_gas_price() -> U256 {
            //TODO: Add gas logic
            (21000 * chui_evm_loader_program::scope::evm::LAMPORTS_TO_GWEI_PRICE
...
async fn main(args: Args) -> StdResult<(), Box<dyn std::error::Error>> {
    ...
    let keyfile_path = args
        .keyfile
        .unwrap_or_else(|| chui_cli_config::Config::default().keypair_path);
...
async fn send_and_confirm_transactions<T: Signers>(
    rpc_client: &AsyncRpcClient,
    mut transactions: Vec<chui::Transaction>,
    signer_keys: &T,
...
mod tests {
    ...
    use chui_sdk::signature::Keypair;
    use std::str::FromStr;
    use std::sync::Arc;
