...
pub enum CliError {
    ...
    #[error("Account {1} has insufficient funds for fee ({0} CUI)")]
    InsufficientFundsForFee(f64, Pubkey),
    #[error("Account {1} has insufficient funds for spend ({0} CUI)")]
    InsufficientFundsForSpend(f64, Pubkey),
    #[error("Account {2} has insufficient funds for spend ({0} CUI) + fee ({1} CUI)")]
...
pub fn parse_command(
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            .gen_completions_to("chui", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands
