...
/// 
/// The `accounts-cluster-bench` program is used to benchmark the performance of creating and closing Chui accounts in a cluster.
/// It utilizes the Chui SDK and RPC client to interact with the Chui network.
/// 
...
use chui_sdk::system_instruction;
use chui_sdk::transaction::Transaction;
use chui_sdk::transport::Result as TransportResult;
use chui_sdk::value::Value;
...
/// 
/// # Arguments
/// * `client` - The RPC client used to interact with the Chui network.
...
use {
    ...
    rand::{thread_rng, Rng},
    rayon::prelude::*,
    chui_account_decoder::parse_token::spl_token_pubkey,
    chui_clap_utils::input_parsers::pubkey_of,
    chui_client::{rpc_client::RpcClient, transaction_executor::TransactionExecutor},
    chui_faucet::faucet::{request_airdrop_transaction, FAUCET_PORT},
    chui_gossip::gossip_service::discover,
    chui_runtime::inline_spl_token,
    chui_sdk::{
    ...
    chui_transaction_status::parse_token::spl_token_instruction,
    std::{
        cmp::min,
        net::SocketAddr,
        process::exit,
...
fn main() {
    ...
    let matches = App::new(crate_name!())
        ...
        fn discover_entrypoint(entrypoint_addr: &SocketAddr) -> TransportResult<SocketAddr> {
            let (gossip_nodes, _) = chui_net_utils::discover(
                None, // keypair
                ...
                None,                    // my_gossip_addr
                0,                       // my_shred_version
                chui_net_utils::SocketAddrSpace::Unspecified,
            )?;
