...
//!
//! Note: The application uses the `chui_streamer` and `chui_net_utils` crates for packet streaming and network utilities respectively.
//! Make sure to include these dependencies in your `Cargo.toml` file.
//!
//! For more information on the `chui_streamer` and `chui_net_utils` crates, please refer to their documentation.
...
fn main() -> Result<()> {
    ...
    for _ in 0..num_sockets {
        let read = chui_net_utils::bind_to(ip_addr, port, false).unwrap();
