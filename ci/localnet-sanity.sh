...
startNodes() {
  ...
  for i in $(seq 0 $((${#nodes[@]} - 1))); do
    ...
    if [[ "$i" -eq 1 ]]; then
      ...
      (
        set -x
        $chui_cli --keypair config/bootstrap-validator/identity.json \
...
killNodes() {
  ...
  echo "--- RPC exit"
  $chui_validator --ledger "$CUI_CONFIG_DIR"/bootstrap-validator exit --force || true
  $chui_validator --ledger "$CUI_CONFIG_DIR"/validator exit --force || true
...
verifyLedger() {
  for ledger in bootstrap-validator validator; do
    ...
    (
      set -x
      $chui_ledger_tool --ledger "$CUI_CONFIG_DIR"/$ledger verify
    ) || flag_error
  done
}
...
if ! $skipSetup; then
  clear_config_dir "$CUI_CONFIG_DIR"
...
while [[ $iteration -le $iterations ]]; do
  ...
  (
    ...
    $chui_keygen new --no-passphrase -fso $client_keypair || exit $?
    $chui_gossip spy -n 127.0.0.1:8001 --num-nodes-exactly $numNodes || exit $?
