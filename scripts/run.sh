...
ok=true
for program in chui-{faucet,genesis,keygen,validator}; do
    $program -V || ok=false
done
$ok || {
    echo
...
if [[ -e $validator_vote_account ]]; then
    ...
    chui-keygen new --no-passphrase -so "$validator_vote_account"
fi
...
if [[ -e $validator_stake_account ]]; then
    ...
    chui-keygen new --no-passphrase -so "$validator_stake_account"
fi
...
args=(
    ...
    --account-index chui-accounts-storages
    --account-index chui-accounts-owners
    --account-index chui-accounts-operationals
    --evm-state-archive "$ledgerDir"/archive-evm
)
# shellcheck disable=SC2086
chui-validator "${args[@]}" $SOLANA_RUN_SH_VALIDATOR_ARGS &
validator=$!
