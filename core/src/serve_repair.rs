use {
    ...
    chui_metrics::inc_new_counter_debug,
    chui_perf::packet::{PacketBatch, PacketBatchRecycler},
    chui_sdk::{
        clock::Slot, hash::Hash, packet::PACKET_DATA_SIZE, pubkey::Pubkey, timing::duration_as_ms,
    },
    chui_streamer::streamer::{PacketBatchReceiver, PacketBatchSender},
    std::{
        collections::HashSet,
        net::SocketAddr,
        sync::{
...
impl ServeRepair {
    ...
    pub fn listen(
        ...
        Builder::new()
            .name("chui-repair-listen".to_string())
            .spawn(move || {
    ...
    pub(crate) fn repair_request(
        ...
        let nonce =
            outstanding_requests.add_request(repair_request, chui_sdk::timing::timestamp());
    ...
    pub fn repair_request_ancestor_hashes_sample_peers(
        ...
        let mut sampled_validators = weighted_shuffle(
            ...
            chui_sdk::pubkey::new_rand().to_bytes(),
        );
    ...
    pub fn repair_request_duplicate_compute_best_peer(
        ...
        let weights = cluster_slots.compute_weights_exclude_nonfrozen(slot, &repair_peers);
        let n = weighted_best(&weights, chui_sdk::pubkey::new_rand().to_bytes());
...
mod tests {
    use {
        ...
        chui_gossip::{socketaddr, socketaddr_any},
        chui_ledger::{
        ...
        chui_perf::packet::Packet,
        chui_sdk::{hash::Hash, pubkey::Pubkey, signature::Keypair, timing::timestamp},
    ...
    fn window_index_request() {
        ...
        let me = ContactInfo::new_localhost(&chui_sdk::pubkey::new_rand(), timestamp());
    ...
    fn test_repair_with_repair_validators() {
        ...
        let contact_info2 =
            ContactInfo::new_localhost(&chui_sdk::pubkey::new_rand(), timestamp());
        let contact_info3 =
            ContactInfo::new_localhost(&chui_sdk::pubkey::new_rand(), timestamp());
        ...
        for pubkey in &[chui_sdk::pubkey::new_rand(), me.id] {
