...
use chui_sdk::account::{ReadableAccount, WritableAccount};
...
pub static ETH_TO_CUI_ADDR: Lazy<H160> = Lazy::new(|| {
    H160::from_str(concat!(
        "56454c41532d434841494e", // 'CHUI-CHAIN'
...
type EthToCuiImp = PromiseFunc<
    fn(Pubkey, NativeContext) -> Result<(PrecompileOutput, u64, Vec<EthToCuiResult>)>,
    fn(AccountStructure, EthToCuiResult) -> Result<()>,
    Pubkey,
    EthToCuiResult,
>;

#[derive(Serialize, Deserialize, Debug)]
pub struct EthToCuiResult {
    pubkey: Pubkey,
    amount: u64,
}

pub static ETH_TO_CUI_CODE: Lazy<NativeContract<EthToCuiImp, Pubkey>> = Lazy::new(|| {
    #[allow(deprecated)]
    let abi = Function {
    ...
    fn implementation(
        pubkey: Pubkey,
        cx: NativeContext,
    ) -> Result<(PrecompileOutput, u64, Vec<EthToCuiResult>)> {
        // EVM should ensure that user has enough tokens, before calling this precompile.

        log::trace!("Precompile ETH_TO_CUI");

        if !matches!(
            cx.precompile_context.call_scheme,
            None | Some(CallScheme::Call)
        ) || cx.precompile_context.evm_context.address != *ETH_TO_CUI_ADDR
    ...
    fn handle_promise(accounts: AccountStructure, promise: EthToCuiResult) -> Result<()> {
        log::trace!("Promise handle ETH_TO_CUI {:?}", promise);
