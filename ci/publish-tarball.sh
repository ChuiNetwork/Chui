...
RELEASE_BASENAME="${RELEASE_BASENAME:=chui-release}"
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml exzo-install-init-"$TARGET"* $MAYBE_TARBALLS; do
  ...
  if [[ -n $BUILDKITE ]]; then
    ...
    # .travis.yml uploads everything in the travis-s3-upload/ directory to release.chui.network
...
# Create install wrapper for release.chui.network
#if [[ -n $DO_NOT_PUBLISH_TAR ]]; then
#  echo "Skipping publishing install wrapper"
#elif [[ -n $BUILDKITE ]]; then
#  cat > release.chui.network-install <<EOF
#CUI_RELEASE=$CHANNEL_OR_TAG
#CUI_INSTALL_INIT_ARGS=$CHANNEL_OR_TAG
#CUI_DOWNLOAD_ROOT=https://release.chui.network
#EOF
#  cat install/chui-install-init.sh >> release.chui.network-install
#
#  echo --- AWS S3 Store: "install"
#  $DRYRUN upload-s3-artifact "/chui/release.chui.network-install" "s3://release.chui.network/$CHANNEL_OR_TAG/install"
#  echo Published to:
#  $DRYRUN ci/format-url.sh https://release.chui.network/"$CHANNEL_OR_TAG"/install
#fi
echo --- ok
