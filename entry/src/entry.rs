...
fn init(name: &OsStr) {
    ...
    unsafe {
        INIT_HOOK.call_once(|| {
            let path;
            let lib_name = if let Some(perf_libs_path) = chui_perf::perf_libs::locate_perf_libs()
            {
                chui_perf::perf_libs::append_to_ld_library_path(
...
mod tests {
    use {
        super::*,
        chui_perf::test_tx::{test_invalid_tx, test_tx},
        chui_sdk::{
            hash::{hash, Hash},
            pubkey::Pubkey,
    ...
    fn test_transaction_signing() {
        use chui_sdk::signature::Signature;
        let zero = Hash::default();
