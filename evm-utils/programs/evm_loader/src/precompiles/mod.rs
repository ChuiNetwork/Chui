...
mod errors;
pub use abi_parse::*;
pub use builtins::{ETH_TO_CUI_ADDR, ETH_TO_CUI_CODE};
...
pub static NATIVE_CONTRACTS: Lazy<HashMap<H160, (NativeBuiltinEval, NativePromiseHandler)>> =
    Lazy::new(|| {
        ...
        let eth_to_sol: NativeBuiltinEval =
            &|function_abi_input, cx| (*ETH_TO_CUI_CODE).eval(function_abi_input, cx);

        let handle_log: NativePromiseHandler = &|accounts, _topics: Vec<H256>, data| {
            (*ETH_TO_CUI_CODE).process_promise(accounts, data)
        };
        assert!(native_contracts
            .insert(*ETH_TO_CUI_ADDR, (eth_to_sol, handle_log))
            .is_none());
        native_contracts
    });
...
pub fn entrypoint(
    ...
    let precompiles = match activate_precompile {
        PrecompileSet::ChuiClassic => &PRECOMPILES_MAP_DEPRECATED,
        PrecompileSet::ChuiNext => &PRECOMPILES_MAP,
...
mod test {
    ...
    fn call_to_sha256() {
        ...
        AccountStructure::testing(0, |accounts: AccountStructure| {
            let precompiles = entrypoint(accounts, PrecompileSet::ChuiClassic, false);
            let input = [0u8; 0];
    ...
    fn call_modexp() {
        ...
        // // this test is slow
        // AccountStructure::testing(0, |accounts| {
        //     let precompiles = entrypoint(accounts, ActivatePrecompile::ChuiNext);
        //     let input = hex!(
        //         "
        ...
        AccountStructure::testing(0, |accounts| {
            let precompiles = entrypoint(accounts, PrecompileSet::ChuiNext, false);
            let input = hex!(
                "
