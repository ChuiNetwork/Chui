...
impl<D: ChuiDBCommon> Clone for Storage<D> {
    fn clone(&self) -> Self {
        Self {
...
impl<D: ChuiDBCommon> std::fmt::Debug for Storage<D> {
...
static SECONDARY_MODE_PATH_SUFFIX: &str = "chui-secondary";
...
pub trait ChuiDBCommon: DBAccess + std::fmt::Debug + Sized {
...
impl ChuiDBCommon for rocksdb::DBWithThreadMode<rocksdb::SingleThreaded> {
...
impl ChuiDBCommon for OptimisticTransactionDB {
    fn flush(&self) -> Result<(), rocksdb::Error> {
        self.flush()
    }
...
pub struct DbWithClose<D: ChuiDBCommon>(D);
...
impl<D: ChuiDBCommon> std::fmt::Debug for DbWithClose<D> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
