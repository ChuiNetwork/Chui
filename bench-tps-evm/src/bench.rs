...
use chui_client::perf_utils::{sample_txs, SampleStats};
use chui_core::gen_keys::GenKeys;
use chui_faucet::faucet::request_airdrop_transaction;
use chui_measure::measure::Measure;
use chui_metrics::{self, datapoint_info};
use chui_sdk::{
    client::Client,
    ...
    T: 'static + Client + Send + Sync,
{
    ...
    Builder::new()
        .name("chui-client-sample".to_string())
        .spawn(move || {
...
pub fn generate_and_fund_keypairs<T: 'static + Client + Send + Sync>(
    ...
    // Sample the first keypair, to prevent lamport loss on repeatedchui-bench-tps executions
...
mod tests {
    ...
    use chui_evm_loader_program::scope::evm::FromKey;
    use chui_runtime::bank::Bank;
    use chui_runtime::bank_client::BankClient;
    use chui_sdk::client::SyncClient;
    use chui_sdk::fee_calculator::FeeRateGovernor;
    use chui_sdk::genesis_config::create_genesis_config;
    ...
    fn test_bench_tps_fund_keys_with_fees() {
        ...
        for (_kp, secret_key) in &keypairs {
            assert_eq!(
                ...
                chui_evm_loader_program::scope::evm::lamports_to_gwei(lamports)
            );
        }
    }
}
