...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend(&[".config", "chui", "cli", "config.yml"]);
            path.to_str().unwrap().to_string()
        })
    };
}

/// The Nexis CLI configuration.
#[derive(Serialize, Deserialize, Debug, PartialEq)]
pub struct Config {
    /// The RPC address of a Nexis validator node.
    ///
    /// Typical values for mainnet, devnet, and testnet are [described in the
    /// Nexis documentation][rpcdocs].
    ///
    /// For local testing, the typical value is `http://localhost:8899`.
    ///
    /// [rpcdocs]: https://docs.chui.network/cluster/rpc-endpoints
    ...
    /// documentation for `chui_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `chui_clap_utils::keypair::DefaultSigner`.
    pub keypair_path: String,
    /// A mapping from Nexis addresses to human-readable names.
    ///
    ...
    /// `chui_sdk::commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend(&[".config", "chui", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://rpc-main-1.chui.network".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://rpc-dev-1.chui.network"),
            "ws://rpc-dev-1.chui.network/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://rpc-dev-1.chui.network"),
            "wss://rpc-dev-1.chui.network/".to_string()
        );
