...
use chui_program_runtime::ic_msg;
use chui_program_runtime::invoke_context::InvokeContext;
use chui_sdk::account::{AccountSharedData, ReadableAccount, WritableAccount};
use chui_sdk::instruction::InstructionError;
use chui_sdk::{keyed_account::KeyedAccount, program_utils::limited_deserialize};
...
impl EvmProcessor {
    pub fn process_instruction(
        ...
        let cross_execution_enabled = invoke_context
            .feature_set
            .is_active(&chui_sdk::feature_set::exzo::evm_cross_execution::id());
        let register_swap_tx_in_evm = invoke_context
            .feature_set
            .is_active(&chui_sdk::feature_set::exzo::native_swap_in_evm_history::id());
        let new_error_handling = invoke_context
            .feature_set
            .is_active(&chui_sdk::feature_set::exzo::evm_new_error_handling::id());
        let ignore_reset_on_cleared = invoke_context
            .feature_set
            .is_active(&chui_sdk::feature_set::exzo::ignore_reset_on_cleared::id());
        let free_ownership_require_signer = invoke_context
            .feature_set
            .is_active(&chui_sdk::feature_set::exzo::free_ownership_require_signer::id());
        let borsh_serialization_enabled = invoke_context
            .feature_set
            .is_active(&chui_sdk::feature_set::exzo::evm_instruction_borsh_serialization::id());
        ...
        if register_swap_tx_in_evm {
            executor.reset_balance(*precompiles::ETH_TO_CUI_ADDR, ignore_reset_on_cleared)
        }
        ...
        let result = match tx {
            ExecuteTransaction::Signed { tx } => {
                ...
                let activate_precompile = precompile_set(
                    ...
                    invoke_context
                        .feature_set
                        .is_active(&chui_sdk::feature_set::exzo::evm_new_precompiles::id()),
                );
    ...
    fn process_free_ownership(
        ...
        if *user.owner() != crate::ID || *user_pk == chui::evm_state::ID {
            ic_msg!(
                invoke_context,
        ...
        user.set_owner(chui_sdk::system_program::id());
        Ok(())
    }
    ...
    fn process_swap_to_evm(
        ...
        if register_swap_tx_in_evm {
            executor.register_swap_tx_in_evm(*precompiles::ETH_TO_CUI_ADDR, evm_address, gweis)
        }
        Ok(())
    }
    ...
    pub fn handle_transaction_result(
        ...
        write!(
            crate::chui_extension::MultilineLogger::new(invoke_context.get_log_collector()),
            "{}",
            result
        )
    ...
    fn check_evm_account<'a>(
        ...
        if first.unsigned_key() != &chui::evm_state::id() || !first.is_writable() {
...
mod test {
    ...
    use chui_program_runtime::{
        invoke_context::{BuiltinProgram, InvokeContext},
        timings::ExecuteTimings,
    };
    use chui_sdk::native_loader;
    use chui_sdk::program_utils::limited_deserialize;
    use chui_sdk::pubkey::Pubkey;
    use chui_sdk::sysvar::rent::Rent;
    use chui_sdk::{
    ...
    struct EvmMockContext {
        ...
        feature_set: chui_sdk::feature_set::FeatureSet,
    }
    ...
    impl EvmMockContext {
        fn new(evm_balance: u64) -> Self {
            ...
            Self {
                ...
                feature_set: chui_sdk::feature_set::FeatureSet::all_enabled(),
            }
        }
        ...
        fn process_transaction(&mut self, ixs: Vec<Instruction>) -> Result<(), InstructionError> {
            ...
            let evm_executor = evm_state::Executor::with_config(
                ...
                evm::EvmConfig::new(
                    evm::TEST_CHAIN_ID,
                    self.feature_set
                        .is_active(&chui_sdk::feature_set::exzo::burn_fee::id()),
                ),
                evm_state::executor::FeatureSet::new(
                    self.feature_set
                        .is_active(&chui_sdk::feature_set::exzo::unsigned_tx_fix::id()),
                    self.feature_set
                        .is_active(&chui_sdk::feature_set::exzo::clear_logs_on_error::id()),
                    self.feature_set.is_active(
                        &chui_sdk::feature_set::exzo::accept_zero_gas_price_with_native_fee::id(
                        ),
                    ),
                ),
            );
            ...
            let evm_program = BuiltinProgram {
                program_id: chui_sdk::evm_loader::id(),
            ...
            for instruction in message.instructions() {
                    ...
                    .result
                {
                    ...
                    let clear_logs = self.feature_set.is_active(
                        &chui_sdk::feature_set::exzo::clear_logs_on_native_error::id(),
                    );
                    self.evm_state
    ...
    fn serialize_deserialize_eth_ix() {
        let tx = dummy_eth_tx();
        {
            let cui_ix = EvmInstruction::new_execute_tx(tx.clone(), FeePayerType::Evm);
            let ser = bincode::serialize(&cui_ix).unwrap();
            assert_eq!(cui_ix, limited_deserialize(&ser).unwrap());
        }
        {
            let cui_ix = EvmInstruction::new_execute_authorized_tx(
                tx.clone().into(),
                H160::zero(),
                FeePayerType::Evm,
            );
        ...
        {
            let cui_ix = EvmInstruction::SwapNativeToEther {
                lamports: 0,
        ...
        {
            let cui_ix = EvmInstruction::FreeOwnership {};
            let ser = bincode::serialize(&cui_ix).unwrap();
            assert_eq!(cui_ix, limited_deserialize(&ser).unwrap());
        }
    }
    ...
    fn execute_tx() {
        ...
        evm_context.disable_feature(&chui_sdk::feature_set::exzo::burn_fee::id());
        let secret_key = evm::SecretKey::from_slice(&SECRET_KEY_DUMMY).unwrap();
    ...
    fn deploy_tx_refund_fee() {
        ...
        assert_eq!(
            evm_context
                .native_account(chui::evm_state::id())
                .borrow()
                .lamports(),
    ...
    fn execute_transfer_to_native_without_needed_account() {
        ...
        let tx_call = evm::UnsignedTransaction {
            ...
            gas_price: 1u32.into(),
            gas_limit: 300000u32.into(),
            action: TransactionAction::Call(*precompiles::ETH_TO_CUI_ADDR),
            value: crate::scope::evm::lamports_to_gwei(lamports_to_send_back),
            input: precompiles::ETH_TO_CUI_CODE
                .abi
        ...
        let tx_call = tx_call.sign(&ether_sc, Some(CHAIN_ID));
        {
            ...
            assert_eq!(
                evm_context
                    .evm_state
                    .get_account_state(*precompiles::ETH_TO_CUI_ADDR)
                    .unwrap()
                    .balance,
                0u32.into()
            )
    ...
    fn all_ixs() -> Vec<chui_sdk::instruction::Instruction> {
        let (tx_call, unsigned_tx) = dummy_call(0);

        let signer = chui::Address::new_unique();
        vec![
    ...
    fn account_by_key(pubkey: chui::Address) -> chui_sdk::account::AccountSharedData {
        match &pubkey {
            id if id == &crate::ID => {
                native_loader::create_loadable_account_for_test("EVM Processor")
            }
            id if id == &chui_sdk::sysvar::rent::id() => chui_sdk::account::Account {
                lamports: 10,
            ...
            .into(),
            _rest => chui_sdk::account::Account {
                lamports: 20000000,
    ...
    #[test]
    fn each_chui_tx_should_contain_writeable_evm_state() {
        for ix in all_ixs() {
        ...
        for code in [&code_without_revert[..], &code_with_revert[..]] {
...

            {
                ...
                if !revert {
                    ...
                    // assert_eq!(lamports, 0); // chuiruntime will revert this account
                }
            }
        }
    }
    ...
    fn test_revert_clears_logs() {
...

        {
            ...
            // Reverted tx with clear_logs_on_error disabled don't clear logs
            {
                let mut evm_context = evm_context.clone(); // make copy for test
                evm_context
                    .disable_feature(&chui_sdk::feature_set::exzo::clear_logs_on_error::id());
    ...
    #[test]
    fn check_tx_mtu_is_in_chuis_limit() {
        use chui_sdk::hash::hash;
        use chui_sdk::message::Message;
        use chui_sdk::signature::{Keypair, Signer};
        use chui_sdk::transaction::Transaction;
