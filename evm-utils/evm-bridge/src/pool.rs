...
use serde_json::json;
use chui_client::{rpc_config::RpcSendTransactionConfig, rpc_request::RpcRequest};
use chui_evm_loader_program::{
    scope::{evm, chui},
    tx_chunks::TxChunks,
};
use chui_sdk::{
...
async fn process_tx(
    ...
    // Shortcut for swap tokens to native, will add chuiaccount to transaction.
    if let TransactionAction::Call(addr) = tx.action {
        use chui_evm_loader_program::precompiles::*;

        if addr == *ETH_TO_CUI_ADDR {
            debug!("Found transferToNative transaction");
            match ETH_TO_CUI_CODE.parse_abi(&tx.input) {
                Ok(pk) => {
    ...
    let mut send_raw_tx: chui::Transaction = chui::Transaction::new_unsigned(message);
...
async fn deploy_big_tx(
    bridge: &EvmBridge,
    payer: &chui_sdk::signature::Keypair,
    tx: &evm::Transaction,
) -> EvmResult<()> {
    let payer_pubkey = payer.pubkey();

    let storage = chui_sdk::signature::Keypair::new();
    ...
    let create_storage_ix = system_instruction::create_account(
        ...
        balance,
        tx_bytes.len() as u64,
        &chui_evm_loader_program::ID,
    );

    let allocate_storage_ix = if bridge.borsh_encoding {
        chui_evm_loader_program::big_tx_allocate(storage_pubkey, tx_bytes.len())
    } else {
        chui_evm_loader_program::big_tx_allocate_old(storage_pubkey, tx_bytes.len())
    };

    let create_and_allocate_tx = chui::Transaction::new_signed_with_payer(
    ...
    let write_data_txs: Vec<chui::Transaction> = tx_bytes
        // TODO: encapsulate
        .chunks(evm_state::TX_MTU)
        .enumerate()
        .map(|(i, chunk)| {
            if bridge.borsh_encoding {
                chui_evm_loader_program::big_tx_write(
                    storage_pubkey,
                    (i * evm_state::TX_MTU) as u64,
                    chunk.to_vec(),
                )
            } else {
                chui_evm_loader_program::big_tx_write_old(
                    storage_pubkey,
                    (i * evm_state::TX_MTU) as u64,
                    chunk.to_vec(),
                )
        ...
        .map(|instruction| {
            chui::Transaction::new_signed_with_payer(
                &[instruction],
                Some(&payer_pubkey),
                &signers,
                blockhash,
    ...
    let execute_tx = chui::Transaction::new_signed_with_payer(
        &instructions,
        Some(&payer_pubkey),
        &signers,
        blockhash,
