use {
    ...
    chui_sdk::{
        clock::UnixTimestamp, hash::Hash, message::Message, native_token::lamports_to_cui,
        program_utils::limited_deserialize, pubkey::Pubkey, stake, transaction::Transaction,
    },
    chui_transaction_status::UiTransactionStatusMeta,
...
pub fn build_balance_message_with_config(
    ...
    let value = if config.use_lamports_unit {
        ...
        letcui= lamports_to_cui(lamports);
        let cui_str = format!("{:.9}", sol);
...
pub fn write_transaction<W: io::Write>(
    ...
    for (instruction_index, instruction) in message.instructions.iter().enumerate() {
        ...
        let mut raw = true;
        if program_pubkey == chui_vote_program::id() {
            if let Ok(vote_instruction) = limited_deserialize::<
                chui_vote_program::vote_instruction::VoteInstruction,
            >(&instruction.data)
            {
        ...
        } else if program_pubkey == chui_sdk::system_program::id() {
            if let Ok(system_instruction) = limited_deserialize::<
                chui_sdk::system_instruction::SystemInstruction,
            >(&instruction.data)
            {
    ...
    if let Some(transaction_status) = transaction_status {
        ...
        writeln!(
            ...
            "{}  Fee: ◎{}",
            prefix,
            lamports_to_cui(transaction_status.fee)
        )?;
        assert_eq!(
...
#[cfg(test)]
mod test {
    use {super::*, chui_sdk::pubkey::Pubkey};
