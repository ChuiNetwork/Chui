# Our understanding of chui (native) runtime.

As a regular blockchain, chui has its transaction format.
Transaction is data transfer object, that user publish in order to change some data in the shared (among parties) state.
In chui state is represented as set of accounts. 
...
Chui names them "programs". Each program has entrypoint, which expects some bytearray, each program decide how they represent this bytearray.

In chui transaction contain multiple instructions. Instruction is a chunk of data that is fed into program entrypoint, with some meta information about account that it uses, and program identificator that should be executed.
...
For state, we understand that chui has same mechanism as in rw-lock. Account metas from transaction define how it would use account state.
...
So for multithread execution, chui just needs to balance transactions among threads, apply rw-lock rule for each account in execution batch, if any of rw-locks rising will fail - transaction execution should be postponed.

## Blockstore
Blockstore in chui is just retrospective information storage, it store shreds(parts of block) for blocks, and multiple metadata indexes.
...
Main requirement was to create a program in chui that can execute EVM bytecode.
...
and evaluate it from computed value, so it is complicated task to separate evm storage into multiple independent parts with rw-lock mechanism as in chui.

Chui has transaction and account size limitation. It expect transaction size to be less than ipv6 MTU 1280 bytes, in order to process and distribute transactions faster.
...
Currently powered is worked by rocksdb mechanism, and it saves all versions that node know in single backup, that then will be shared among nodes, trough chui snapshots.
...
When chui create new bank (at slot beginning) state is again reset to `Empty`.
...
Save block: For saving blocks in blockstore, we go the same way as chui goes - after bank finalization background service EvmRecorderService receive block from `Committed` state and save it in blockstore.
...
As result it provide methods that can be called in evm_loader, some evm specific contracts are hardcoded in calls, and this method are marked as failable, other chui specific are implemented in evm_loader.
...
# Evm loader program.
Evm loader program is chui native program with custom entrypoint that execute next instruction types:

1) `EvmTransaction` - execute regular evm transaction, it is limited to be less than 900 bytes in bincode encoding, internally call `Executor::transaction_execute`. As extension to regular transaction flow, we implement precompile module, wich provide evm specific precompile, and swap from evm to chui world code.
2) `SwapNativeToEther` - transfer chui lamports to evm world. Internally it just take lamports from user account, transfer it to EvmState account in chui world, and mint new tokens in evm world for specific account.
...
4) `EvmBigTransaction` - Big transaction functionality is done like in chui bpf program deploy. User create account that assign to evm program, write batches trough `EvmTransactionWrite` and after this call `EvmTransactionExecute` to ask evm_loader to load transaction from account and execute it using 1), or `EvmTransactionExecuteUnsigned` to execute it using 5)
5) `EvmAuthorizedTransaction` - Execute evm transaction that is created by chui program. In evm world we allocate addresses started with `0xacc0`, check method `evm_address_for_program` for more details how we allocate addresses. This transaction is signed in chui, so we check is_signed flag, and execute it trough `Executor::transaction_execute_unsigned`.
   
Swap from evm implementation:
Swap from evm world is done by implementing precompile that modify chui state,
...
- Both our swaps works invalid with revert. `SwapNativeToEther` with revert in chui can keep money on evm, and reverts in evm didn't trigger revert in chui. To fix this, now revert in evm trigger revert in chui, and on any revert, state changes are ignored for whole transaction execution.