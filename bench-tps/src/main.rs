...
//!
//! The `bench-tps` application is used to benchmark the transaction processing speed (TPS) of the Chui blockchain network.
//! It generates and executes a large number of transactions using multiple client nodes, measuring the TPS achieved.
//!
//! The application uses the `chui_bench_tps` and `chui_genesis` crates for benchmarking and generating keypairs respectively.
//! It also relies on the `chui_gossip` crate for discovering the cluster nodes, and the `chui_sdk` crate for interacting with the Chui blockchain.
//!
...
//!
//! For more information on how to use the `bench-tps` application, refer to the documentation of the `chui_bench_tps` crate.
//!
//! Example usage:
//!
//!
...
use {
    ...
    chui_genesis::Base64Account,
    chui_gossip::gossip_service::{discover_cluster, get_client, get_multi_client},
    chui_sdk::{
...
fn main() {
    ...
    chui_metrics::set_panic_hook("bench-tps", /*version:*/ None);
