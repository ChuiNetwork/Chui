...
(
  ...
  set -x
  wget http://api.mainnet-beta.chui.network/genesis.tar.bz2
  wget --trust-server-names http://api.mainnet-beta.chui.network/snapshot.tar.bz2
)
...
if [[ -r $FAUCET_KEYPAIR ]]; then
  ...
  $chui_keygen new --no-passphrase -fso "$SOLANA_CONFIG_DIR"/faucet.json
fi

if [[ -f $BOOTSTRAP_VALIDATOR_IDENTITY_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_IDENTITY_KEYPAIR" "$SOLANA_CONFIG_DIR"/bootstrap-validator/identity.json
else
  $chui_keygen new --no-passphrase -so "$SOLANA_CONFIG_DIR"/bootstrap-validator/identity.json
fi

$chui_keygen new --no-passphrase -so "$SOLANA_CONFIG_DIR"/bootstrap-validator/vote-account.json
$chui_keygen new --no-passphrase -so "$SOLANA_CONFIG_DIR"/bootstrap-validator/stake-account.json

$chui_ledger_tool create-snapshot \
...
$chui_ledger_tool modify-genesis \
  --ledger "$SOLANA_CONFIG_DIR"/latest-mainnet-beta-snapshot \
  --hashes-per-tick sleep \
