use {
    ...
    log::*,
    chui_account_decoder::{UiAccountEncoding, UiDataSliceConfig},
    chui_bpf_loader_program::{syscalls::register_syscalls, BpfError, ThisInstructionMeter},
    ...
    chui_program_runtime::invoke_context::InvokeContext,
    chui_rbpf::{elf::Executable, verifier, vm::Config},
                    ...
                    SubCommand::with_name("deploy")
                        ...
                        .arg(
                            Arg::with_name("allow_excessive_balance")
                                ...
                                .takes_value(false)
                                .help("Use the designated program id even if the account already holds a large balance of CUI")
                        ),
                )
                .subcommand(
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic) {
    ...
    eprintln!("[BUFFER_SIGNER] to `chuiprogram deploy` or `chuiwrite-buffer'.");
    eprintln!("Or to recover the account's lamports, pass it as the");
    eprintln!(
        "[BUFFER_ACCOUNT_ADDRESS] argument to `chuiprogram close`.\n{}",
        divider
    );
}
...
mod tests {
    use {
        ...
        serde_json::Value,
        chui_cli_output::OutputFormat,
        chui_sdk::signature::write_keypair_file,
    };
